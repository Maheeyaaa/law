...
function ProviderBadge({ courtType }: { courtType: string }) {
  ...
  const badgeMap: Record<string, {
    ...
    tshc:     { label: "Telangana HC Portal", color: "#a78bfa", bg: "rgba(139,92,246,.1)",  border: "rgba(139,92,246,.3)"  },
...
function ComingSoonBanner({ courtType }: { courtType: string }) {
  ...
  return (
    <div style={{
      ...GLASS, borderRadius: 14, padding: "20px 24px",
...
export default function TrackStatus() {
  ...
  // ══════════════════════════════════════════════════════════════
  // ✅ NEW — Load saved case from URL param (notification click)
  // ══════════════════════════════════════════════════════════════
  const loadSavedCaseAndTrack = async (savedCaseId: string) => {
    try {
      ...
      // Use credentials object if available (richer), else fall back to savedCase
      const data = creds || sc;
      ...
      // Show cached tracking data if backend returned it
      if (res.data?.cachedTrackingData) {
        setTrackingData(res.data.cachedTrackingData);
      }

      // Load dropdowns if eCourts
  ...
  useEffect(() => {
    ...
    // ✅ NEW — Check URL param first (from notification click)
  ...
  return (
    <CitizenLayout activeNav="track">
      ...
      <div style={{ padding: "28px 28px 60px", display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
          <div>
            ...
            <p style={{ ...DM, fontSize: 12, color: "rgba(255,255,255,.35)", marginTop: 6 }}>
              Track cases across Telangana High Court and all District Courts.
            </p>
          </div>
          <button
        ...
        <div style={{ ...GLASS, borderRadius: 18, padding: "28px" }}>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            <button
              ...
              style={{
                ...DM,
                background: loading || !courtTypeConfig?.supported ? "rgba(30,95,255,.4)" : BLUE,
            ...
            {(trackingData || requiresCNR || error) && (
              <button
                ...
                style={{
                  ...
                  fontSize: 12, padding: "12px 20px", borderRadius: 11, cursor: "pointer",
                }}
              >
                Clear
              </button>
          ...
          {error && (
            <div style={{
              ...
              borderRadius: 9, padding: "11px 14px", ...DM, fontSize: 12, color: "#ef4444",
            }}>
              ✕ {error}
            </div>
          )}
        ...
        {requiresCNR && (
          <div style={{
            ...GLASS, borderRadius: 16, padding: "24px",
