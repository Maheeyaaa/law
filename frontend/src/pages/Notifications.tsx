...
// Display label for notification type
function typeLabel(type: string): string {
  switch (type) {
    case "hearing":  return "Consultation";
    case "case":     return "Request";
...
export default function Notifications() {
  ...
  const groupByDate = (notifs: any[]) => {
    ...
    const now = new Date();
    notifs.forEach(n => {
      const diff = Math.floor((now.getTime() - new Date(n.createdAt).getTime()) / 86400000);
      ...
      <div style={{ padding: "28px 28px 60px", display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {unreadCount > 0 && (
              <button onClick={handleMarkAllRead}
                style={{ ...DM, background: "rgba(30,95,255,.15)", color: BLUEB, fontSize: 11, fontWeight: 600, padding: "8px 16px", borderRadius: 10, border: "1px solid rgba(30,95,255,.3)", cursor: "pointer", transition: "all .2s ease" }}
            ...
            {notifications.some(n => n.read) && (
              <button onClick={handleClearRead}
                style={{ ...DM, background: "rgba(239,68,68,.15)", color: "#ef4444", fontSize: 11, fontWeight: 600, padding: "8px 16px", borderRadius: 10, border: "1px solid rgba(239,68,68,.3)", cursor: "pointer", transition: "all .2s ease" }}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
          {[
            ...
            { label: "Consultations", value: notifications.filter(n => n.type === "hearing").length, color: "#3b82f6"  },
            { label: "Requests",      value: notifications.filter(n => n.type === "case").length,    color: "#34d399"  },
            { label: "Documents",     value: notifications.filter(n => n.type === "document").length, color: "#fbbf24" },
          ].map((stat, i) => (
            <div key={i}
              style={{ ...GLASS, borderRadius: 14, padding: "16px 18px", position: "relative", overflow: "hidden", transition: "transform .2s ease" }}
        ...
        <div style={{ ...GLASS, borderRadius: 16, padding: "16px 24px", display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          {[
            ...
            <button key={f.id} onClick={() => setFilter(f.id)}
              style={{ ...DM, fontSize: 11, fontWeight: 600, padding: "7px 14px", borderRadius: 99, cursor: "pointer", background: filter === f.id ? BLUE : "rgba(30,95,255,0.15)", color: "#fff", border: filter === f.id ? "none" : "1px solid rgba(30,95,255,0.4)", transition: "all .2s ease" }}>
              {f.label}
            </button>
          ))}
        </div>
                ...
                <div key={group}>
                  ...
                  <div style={{ ...GLASS, borderRadius: 16, overflow: "hidden" }}>
                    {grouped[group].map((n: any, i: number) => (
                      <div key={n._id}
                        style={{ display: "flex", alignItems: "flex-start", gap: 14, padding: "16px 20px", borderBottom: i < grouped[group].length - 1 ? "1px solid rgba(255,255,255,.04)" : "none", background: !n.read ? "rgba(30,95,255,.05)" : "transparent", transition: "background .2s ease", cursor: "pointer" }}
                        ...
                        <div style={{ flex: 1, minWidth: 0 }} onClick={() => { if (!n.read) handleMarkRead(n._id); }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                            <p style={{ ...DM, fontSize: 13, fontWeight: !n.read ? 700 : 500, color: !n.read ? "#fff" : "rgba(255,255,255,.6)" }}>
                              {n.title}
                            </p>
                            {!n.read && (
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 6 }}>
                            <span style={{ ...DM, fontSize: 9, padding: "2px 8px", borderRadius: 99, background: `${typeColor(n.type)}15`, color: typeColor(n.type) }}>
                              {typeLabel(n.type)}
                            </span>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 6, flexShrink: 0 }}>
                          {!n.read && (
                            <button
                              ...
                              style={{ ...DM, background: "rgba(30,95,255,.15)", color: BLUEB, fontSize: 9, fontWeight: 600, padding: "4px 10px", borderRadius: 6, border: "1px solid rgba(30,95,255,.25)", cursor: "pointer", whiteSpace: "nowrap" }}
                              onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = "rgba(30,95,255,.25)"}
                              onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = "rgba(30,95,255,.15)"}>
                              Mark Read
                            </button>
                          ...
                          <button
                            ...
                            style={{ ...DM, background: "rgba(239,68,68,.1)", color: "#ef4444", fontSize: 9, fontWeight: 600, padding: "4px 10px", borderRadius: 6, border: "1px solid rgba(239,68,68,.2)", cursor: "pointer", whiteSpace: "nowrap" }}
