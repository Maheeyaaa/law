...
function categoryColor(category: string): string {
  switch (category) {
    ...
    case "Consultations": return "#3b82f6";
            ...
            <div style={{ ...GLASS, borderRadius: 16, padding: "20px 24px", display: "flex", alignItems: "center", gap: 16 }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                <button
                  ...
                  style={{ ...DM, fontSize: 10, fontWeight: 600, padding: "6px 14px", borderRadius: 99, cursor: "pointer", background: faqCategory === "" ? BLUE : "rgba(30,95,255,0.15)", color: "#fff", border: faqCategory === "" ? "none" : "1px solid rgba(30,95,255,0.4)", transition: "all .2s ease" }}>
                  All
                </button>
                {categories.map(c => (
                  <button key={c} onClick={() => setFaqCategory(c)}
                    style={{ ...DM, fontSize: 10, fontWeight: 600, padding: "6px 14px", borderRadius: 99, cursor: "pointer", background: faqCategory === c ? BLUE : "rgba(30,95,255,0.15)", color: "#fff", border: faqCategory === c ? "none" : "1px solid rgba(30,95,255,0.4)", transition: "all .2s ease" }}>
                    {categoryIcon(c)} {c}
                  </button>
                ))}
              </div>
                ...
                {Object.keys(groupedFaqs).map(category => (
                  <div key={category}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, paddingLeft: 4 }}>
                      ...
                      <p style={{ ...DM, fontSize: 11, letterSpacing: "2px", textTransform: "uppercase", color: categoryColor(category), fontWeight: 600 }}>
                        {/* Show "Consultations" instead of "Hearings" in display */}
                        {category === "Hearings" ? "Consultations" : category}
                      </p>
                    </div>
                        ...
                        return (
                          <div key={faq._id} style={{ borderBottom: i < groupedFaqs[category].length - 1 ? "1px solid rgba(255,255,255,.04)" : "none" }}>
                            <div
                              ...
                              style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px", cursor: "pointer", transition: "background .2s ease" }}
                              onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = "rgba(30,95,255,.05)"}
                              onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = "transparent"}>
                              <p style={{ ...DM, fontSize: 13, fontWeight: 600, color: isOpen ? "#fff" : "rgba(255,255,255,.7)", flex: 1 }}>
                                {faq.question}
                              </p>
                              <span style={{ ...DM, fontSize: 16, color: "rgba(255,255,255,.3)", transform: isOpen ? "rotate(180deg)" : "rotate(0deg)", transition: "transform .2s ease", flexShrink: 0, marginLeft: 12 }}>
                                ▾
                              </span>
                            </div>
                            {isOpen && (
          ...
          <div style={{ display: "flex", gap: 24 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ ...GLASS, borderRadius: 20, padding: "28px", position: "relative", overflow: "hidden" }}>
                ...
                <p style={{ ...DM, fontSize: 12, color: "rgba(255,255,255,.35)", marginBottom: 24 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  <div>
                    ...
                    <input
                      ...
                      style={{ ...DM, width: "100%", background: "rgba(255,255,255,.04)", border: "1px solid rgba(30,95,255,.25)", borderRadius: 10, padding: "12px 16px", color: "rgba(255,255,255,.6)", fontSize: 13, outline: "none", boxSizing: "border-box" }}
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      style={{ ...DM, width: "100%", background: "rgba(255,255,255,.04)", border: "1px solid rgba(30,95,255,.25)", borderRadius: 10, padding: "12px 16px", color: "rgba(255,255,255,.6)", fontSize: 13, outline: "none", resize: "none", boxSizing: "border-box" }}
                    />
                  </div>
                  <button
                    onClick={handleSubmitContact}
                    disabled={sending}
                    style={{ ...DM, background: sending ? "rgba(30,95,255,.4)" : BLUE, color: "#fff", fontSize: 13, fontWeight: 600, padding: "12px 24px", borderRadius: 10, border: "none", cursor: sending ? "not-allowed" : "pointer", boxShadow: SH_CARD, transition: "transform .2s ease", alignSelf: "flex-start" }}
            ...
            <div style={{ width: 320, flexShrink: 0, display: "flex", flexDirection: "column", gap: 16 }}>
              {[
                ...
                <div key={i}
                  style={{ ...GLASS, borderRadius: 14, padding: "18px 20px", display: "flex", alignItems: "flex-start", gap: 14, transition: "transform .2s ease" }}
              ...
              {/* NALSA Info */}
              <div style={{ ...GLASS, borderRadius: 14, padding: "18px 20px", border: "1px solid rgba(52,211,153,.2)" }}>
                <p style={{ ...DM, fontSize: 12, fontWeight: 600, color: "#34d399", marginBottom: 8 }}>
                  🏛️ Free Legal Aid
                </p>
                <p style={{ ...DM, fontSize: 11, color: "rgba(255,255,255,.5)", lineHeight: 1.7 }}>
                  Need free legal assistance? Call NALSA helpline:
                </p>
                <p style={{ ...DM, fontSize: 20, fontWeight: 700, color: "#34d399", marginTop: 8 }}>15100</p>
            ...
            {messagesLoading ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {supportMessages.map((m) => (
                  <div key={m._id}
                    style={{ padding: "18px 20px", borderRadius: 12, background: "rgba(0,0,0,0.4)", border: "1px solid rgba(255,255,255,.06)", transition: "all .2s ease" }}
                    ...
                    <p style={{ ...DM, fontSize: 12, color: "rgba(255,255,255,.4)", lineHeight: 1.7, marginBottom: 8 }}>
                      {m.message}
                    </p>

                    <p style={{ ...DM, fontSize: 9, color: "rgba(255,255,255,.2)" }}>
                      Submitted on {formatDate(m.createdAt)}
                    </p>
                  </div>
                ))}
              </div>
