...
const BLUE = "#1748cf"; // 10% deeper than #1e5fff
const BLUEB = "#1748cf";
const ICEB = "#1748cf";

const SH_CARD = "0 8px 32px rgba(0,0,0,.7)";

// UPDATED: Added faint blue border here
const GLASS_BOX1 = {
...
function StatCard({ value, label }: { value: string; label: string }) {
  return (
    ...
    >
      ...
      <p style={{ ...DM, fontSize: 7.5, color: "rgba(140,180,255,.7)", textTransform: "uppercase", letterSpacing: "1px", marginTop: 2, marginBottom: 0 }}>{label}</p>
    </div>
  );
}

/* ──────────── Plate Component ──────────── */
function Plate({ children, style, id, glassStyle }: {
  children: ReactNode;
  style?: CSSProperties;
  id?: string;
...
export default function CitizenDashboard() {
  ...
  const handleNav = (id: string) => {
    setActiveTab(id);
    if (id === "dash") { sec0.current?.scrollIntoView({ behavior: "smooth", block: "start" }); return; }
    if (id === "ai") { navigate("/citizen/legal-chatbot"); return; }
    if (id === "file") { sec1.current?.scrollIntoView({ behavior: "smooth", block: "start" }); return; }
    if (id === "cases") { navigate("/citizen/cases"); return; }
    if (id === "law") { navigate("/citizen/find-lawyer"); return; }
    if (id === "docs") { navigate("/citizen/documents"); return; }
    if (id === "hear" || id === "track") { sec2.current?.scrollIntoView({ behavior: "smooth", block: "start" }); return; }
    navigate("/citizen");
  };
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      } catch { console.log("Failed to parse user data"); }
    }
    ...
    const fetchDashboard = async () => {
      ...
      } catch (err) { console.log("Failed to load dashboard data"); }
          ...
          <div style={{ padding: "13px", display: "flex", flexDirection: "column", gap: 28 }}>
            ...
            <nav 
              ...
              style={{
                ...
                border: "1px solid rgba(30, 95, 255, 0.3)", // UPDATED: Faint blue border
                borderRadius: 26,
                padding: "18px 24px",
            ...
            <div ref={sec0} className="a0" style={{ position: "relative", width: "100%", paddingTop: 70 }}>
              ...
              <div style={{ width: "42%", minWidth: 320, position: "absolute", top: 0, left: 18, zIndex: 20 }}>
                ...
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
                    stroke="rgba(180,210,255,.75)" strokeWidth="2.5"
                  ...
                  />
                  <span style={{
                    ...DM, fontSize: 11, color: "rgba(255,255,255,.4)", flexShrink: 0,
                ...
                {searchResults && searchQuery.trim().length >= 2 && (
                  <div style={{
                    ...
                    boxShadow: SH_CARD, zIndex: 100, maxHeight: 280, overflowY: "auto",
                  }}>
                          ...
                          <div style={{ marginBottom: 8 }}>
                            ...
                            {searchResults.cases.map((c: any) => (
                              <div key={c._id}
                                ...
                                onClick={() => { setSearchQuery(""); setSearchResults(null); navigate(`/citizen/cases/${c._id}`); }}>
                          ...
                          <div>
                            ...
                            {searchResults.documents.map((d: any) => (
                              <div key={d._id}
                                ...
                                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = "rgba(30,95,255,.1)"; }}
                                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = "transparent"; }}
                                onClick={() => { setSearchQuery(""); setSearchResults(null); navigate("/citizen/documents"); }}>
              ...
              {/* BOX 1 Plate - Uses GLASS_BOX1 (Blue Border) */}
              <Plate glassStyle={GLASS_BOX1} style={{ padding: "70px 38px 48px", marginLeft: 18 }}>
                <div style={{ display: "flex", gap: 28, alignItems: "stretch" }}>
                  ...
                  <div style={{
                    ...
                    justifyContent: "space-between", padding: "8px 0", gap: 36,
                  }}>
                    <div>
                      <p style={{ ...DM, fontSize: 11, letterSpacing: "3px", textTransform: "uppercase", color: "rgba(200,220,255,.85)", marginBottom: 18, fontWeight: 600 }}>
                                WELCOME BACK{user?.name ? `, ${user.name.split(" ")[0].toUpperCase()}` : ""}
                      </p>
                      <p style={{ ...DM, fontSize: 14, color: "rgba(255,255,255,.65)", lineHeight: 1.7, maxWidth: 460 }}>
                        {casesList.length > 0 ? (
                          <>
                                Your legal requests are being processed.
                          {nextHearingDate && <> Next consultation on <span style={{ color: ICEB, fontWeight: 600 }}>{nextHearingDate}</span></>}
                          {pendingDocs > 0 && <> and {pendingDocs} doc{pendingDocs > 1 ? "s" : ""} await review</>}
                          .
                          </>
                        ) : (
                    ...
                    <div style={{ display: "flex", gap: 12 }}>
                      <button
                        ...
                        style={{ ...DM, background: BLUE, color: "#fff", fontSize: 13, fontWeight: 600, padding: "12px 22px", borderRadius: 999, border: "none", cursor: "pointer", boxShadow: "none", whiteSpace: "nowrap", transition: "transform .2s ease" }}
                        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(-3px)"; }}
                        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(0)"; }}
                      >
                                Submit request
                      ...
                      <button
                        ...
                        style={{ ...DM, background: "rgba(0,0,0,0.6)", color: "rgba(255,255,255,.85)", fontSize: 13, fontWeight: 500, padding: "12px 22px", borderRadius: 999, border: "none", cursor: "pointer", whiteSpace: "nowrap", transition: "transform .2s ease", boxShadow: "0 6px 16px rgba(0,0,0,.55)" }}
                        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(-3px)"; }}
                        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(0)"; }}
                      >
                                View All
                  ...
                  {/* RIGHT — Case Status */}
                  <div style={{
                    flex: "1 1 0", minWidth: 360,
                    background: INNER_SOLID,
                    border: "none",
                    borderRadius: 18,
                    padding: "22px 26px 38px",
                    display: "flex", flexDirection: "column", justifyContent: "center",
                    boxShadow: SH_CARD, position: "relative",
                  }}>
                    <p style={{ ...DM, fontSize: 11, letterSpacing: "3px", textTransform: "uppercase", color: "rgba(200,220,255,.7)", marginBottom: 14, fontWeight: 600 }}>CASE STATUS</p>
                    <p style={{ ...DM, fontSize: 14, color: "rgba(255,255,255,.65)", lineHeight: 1.7 }}>
                            ...
                            {nextHearingDate && <> Next consultation is scheduled for <span style={{ color: ICEB, fontWeight: 600 }}>{nextHearingDate}</span>.</>}
                            {pendingDocs > 0 && <> {pendingDocs} doc{pendingDocs > 1 ? "s are" : " is"} pending verification.</>}
                    </p>
                    <div style={{ position: "absolute", bottom: -16, left: 0, right: 0, display: "flex", justifyContent: "center", gap: 14, zIndex: 30, flexWrap: "wrap" }}>
            ...
            <div id="submit-request" ref={sec1} style={{ marginLeft: 18 }}>
              <Plate glassStyle={GLASS_BOX234} style={{ padding: "70px 38px 48px" }}>
                <div style={{ display: "flex", gap: 28, alignItems: "stretch" }}>
                  ...
                  {/* My Requests */}
                  <div id="my-requests" style={innerCard}
                    ...
                    <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
                      {["All", "Active", "Pending", "Resolved"].map(t => (
                        <span key={t} onClick={() => setCaseFilter(t)}
                          style={{ ...DM, fontSize: 10, fontWeight: 600, padding: "4px 12px", borderRadius: 99, cursor: "pointer", background: caseFilter === t ? BLUE : "rgba(30,95,255,0.15)", color: "#fff", border: "none", transition: "all .2s ease" }}>
                          {t}
                        </span>
                      ))}
                    </div>
                    ...
                    <div style={{ maxHeight: 180, overflowY: "auto", paddingRight: 4 }}>
                      {filteredCases.length === 0 ? (
                        <div style={{ textAlign: "center", padding: 20 }}>
                          ...
                          <button onClick={() => handleNav("file")}
                            style={{ ...DM, fontSize: 10, color: BLUEB, background: "none", border: "none", cursor: "pointer", marginTop: 8 }}>
                                Submit your first request
                          </button>
                        </div>
                      ) : filteredCases.map((r, i) => (
                        <div key={r._id || i}
                          style={{ display: "grid", gridTemplateColumns: "1fr 2fr 1fr 1fr", gap: 8, padding: "10px 10px", borderRadius: 10, background: i % 2 === 0 ? "rgba(255,255,255,.03)" : "transparent", alignItems: "center", marginBottom: 2 }}>
                          <p style={{ ...DM, fontSize: 11, color: BLUEB, fontWeight: 600 }}>{r.caseId}</p>
                          <p style={{ ...DM, fontSize: 11, color: "rgba(255,255,255,.6)" }}>{r.title}</p>
                          <span style={{ ...DM, fontSize: 9, fontWeight: 600, padding: "3px 8px", borderRadius: 99, background: "rgba(30,95,255,0.2)", border: "none", color: "#6aadff", display: "inline-block" }}>{r.status}</span>
                          <span onClick={() => navigate(`/citizen/cases/${r._id}`)} style={{ ...DM, fontSize: 10, color: BLUEB, cursor: "pointer" }}>View</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={{ ...innerCard, display: "flex", flexDirection: "column", gap: 14 }}
                    onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(-4px)"; }}
                    onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(0)"; }} >
                    <div>
                      <p style={{ ...DM, fontSize: 13, fontWeight: 600, color: "rgba(200,220,255,.85)", marginBottom: 4 }}>Submit Legal Request</p>
                    ...
                    {fileMsg && (
                      <div style={{
                                ...
                                border: `1px solid ${fileMsg.type === "success" ? "rgba(52,211,153,.3)" : "rgba(255,107,107,.3)"}`,
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      <div>
                        ...
                        <select value={fileForm.caseType} onChange={e => setFileForm({ ...fileForm, caseType: e.target.value })} style={{ ...inp, cursor: "pointer" }}>
                      ...
                      <div>
                        ...
                        <select value={fileForm.district} onChange={e => setFileForm({ ...fileForm, district: e.target.value, courtName: "" })} style={{ ...inp, cursor: "pointer" }}>
                      ...
                      <div>
                        ...
                        <select value={fileForm.courtName} onChange={e => setFileForm({ ...fileForm, courtName: e.target.value })} style={{ ...inp, cursor: "pointer" }}>
                          <option value="">Select Court</option>
                          {telanganaCourts.filter(c => !fileForm.district || c.district === fileForm.district).map(c => <option key={c.name} value={c.name}>{c.name}</option>)}
                        </select>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        {supportingDocs.length > 0 && (
                          <div style={{ marginTop: 10 }}>
                            {supportingDocs.map((f, i) => <p key={i} style={{ ...DM, fontSize: 10, color: BLUEB }}>{f.name}</p>)}
                          </div>
                        )}
                      </div>
                      <button
                        ...
                        disabled={filing}
                        style={{ ...DM, background: filing ? "rgba(30,95,255,.5)" : BLUE, color: "#fff", fontSize: 12, fontWeight: 600, padding: "10px 16px", borderRadius: 9, border: "none", cursor: filing ? "not-allowed" : "pointer", boxShadow: "none", transition: "transform .2s ease" }}
            ...
            <div ref={sec2} style={{ marginLeft: 18 }}>
              ...
              {/* AI Legal Assistant Banner - Uses GLASS_BOX234 (Blue Border) */}
              <Plate glassStyle={GLASS_BOX234} id="ai-section" style={{ padding: "70px 38px 48px", marginBottom: 28 }}>
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
                    <div style={{ width: 52, height: 52, borderRadius: 14, background: "linear-gradient(135deg,#1e5fff,#4d8aff)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: "bold", color: "#fff", flexShrink: 0 }}>AI</div>
                    <div>
                      <p style={{ ...DM, fontSize: 16, fontWeight: 700, color: "#fff", marginBottom: 4 }}>AI Legal Assistant</p>
                      <p style={{ ...DM, fontSize: 12, color: "rgba(255,255,255,.4)", lineHeight: 1.6 }}>Get instant help with legal notices, deadlines, document checklists, scam detection & more</p>
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", maxWidth: 340 }}>
                      {["Notice Explainer", "Deadlines", "Legal Terms", "Scam Detector", "Filing Guide", "Doc Checklist", "Case Insights"].map(pill => (
                        <span key={pill} style={{ ...DM, fontSize: 9, padding: "4px 10px", borderRadius: 20, background: "rgba(30,95,255,.12)", border: "none", color: ICEB, whiteSpace: "nowrap" }}>{pill}</span>
                      ))}
                    </div>
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: BLUE, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, color: "#fff", flexShrink: 0, boxShadow: SH_CARD, marginLeft: 8 }}>&rarr;</div>
                  </div>
                </div>
              </Plate>

              {/* 3-Column Section - Uses GLASS_BOX234 (Blue Border) */}
                ...
                <div style={{ display: "flex", gap: 28, alignItems: "stretch" }}>
                  ...
                  <div style={innerCard}
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 10, maxHeight: 180, overflowY: "auto" }}>
                      {activitiesList.length === 0 ? (
                        <p style={{ ...DM, fontSize: 11, color: "rgba(255,255,255,.3)", textAlign: "center", padding: 20 }}>No recent activity</p>
                      ) : activitiesList.map((a, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                          <span style={{ width: 6, height: 6, borderRadius: "50%", background: BLUEB, flexShrink: 0, marginTop: 4 }} />
                          <div>
                  ...
                  <div style={innerCard}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                      ...
                      <span onClick={() => navigate("/citizen/find-lawyer")} style={{ ...DM, fontSize: 10, color: BLUEB, cursor: "pointer" }}>Find More</span>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 10, maxHeight: 180, overflowY: "auto" }}>
                      {lawyersList.length === 0 ? (
                        <div style={{ textAlign: "center", padding: 20 }}>
                          <p style={{ ...DM, fontSize: 11, color: "rgba(255,255,255,.3)" }}>No lawyers assigned</p>
                          <span onClick={() => navigate("/citizen/find-lawyer")} style={{ ...DM, fontSize: 10, color: BLUEB, cursor: "pointer" }}>Find a lawyer</span>
                        </div>
                      ) : lawyersList.map((l, i) => (
                  ...
                  {/* My Documents */}
                  <div id="documents-section" style={innerCard}
                    onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(-4px)"; }}
                    onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(0)"; }} >
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 10, maxHeight: 180, overflowY: "auto" }}>
                      {docsList.length === 0 ? (
                        <div style={{ textAlign: "center", padding: 20 }}>
                          ...
                          <span onClick={() => navigate("/citizen/documents")} style={{ ...DM, fontSize: 10, color: BLUEB, cursor: "pointer" }}>Upload documents</span>
                        </div>
                      ) : docsList.map((d, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            <span style={{ ...DM, fontSize: 9, padding: "2px 6px", borderRadius: 4, background: "rgba(30,95,255,.15)", border: "none", color: BLUEB }}>{d.fileType}</span>
